<template>
  <div class="px-3 w-screen py-6 inline-flex flex-row items-center justify-between xs:space-x-0 sm:space-x-0 space-x-12">
    ...
    <div class="flex container max-w-6xl mx-auto px-8 space-y-8 rounded-md bg-white md:pr-0 lg:pr-0 xs:pb-8">
      <div v-if=currentToken class="max-w-6xl mx-auto grid xs:grid-cols-1 sm:grid-cols-1 grid-cols-2 gap-8 xs:text-center filter shadow-xl rounded-md">
        <div class="xs:mt-4 pt-4 xs:pl-0 mt-10 pl-10">
          <router-link :to='`/art/${currentToken.id}`' class="text-gray-800 xs:text-xl text-2xl font-bold capitalize leading-relaxed w-10">
        ...
        <img class="max-w-screen-lg h-auto content-end xs:filter xs:drop-shadow-xl" :src="`https://ipfs.io/ipfs/${currentToken.token_uri}`">
      </div>
    </div>
