<template>
  <div class="art">
    ...
    <div class=" bg-gray-50 mt-10 m-3 px-6 pb-5 rounded filter shadow-md container max-w-7xl sm:px-4 xs:px-3 mx-auto grid xs:grid-cols-1 sm:grid-cols-1 grid-cols-2 pt-6 gap-8 xs:gap-7 xs:justify-items-center">
      <img class="max-w-screen-lg h-auto" :src="`${token.token_uri}`">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 mt-12 bg-white rounded-md xs:space-x-2 divide-x filter drop-shadow px-3">
          <div class="space-y-4 space-y-2 px-2 my-3">
            ...
            <div v-if='token.data.createdAt' class="uppercase tracking-widest text-sm xs:text-xs">Created At: {{ new Date(token.data.createdAt * 1000).toISOString().split('T')[0] }}</div>
      ...
      <div class="xs:bg-white filter drop-shadow rounded-md w-full py-3 px-6 xs:pb-6">
        ...
        <div class=" mt-5 flex flex-cols space-x-10">
          <div v-if='token.royalties'>
            <div v-if='token.royalties.roylatiesArtist' class="uppercase tracking-widest text-xs  ">Artist: {{ token.royalties.roylatiesArtist }} %</div>
            <div v-if='token.royalties.roylatiesGallery' class="uppercase tracking-widest text-xs ">Gallery: {{ token.royalties.roylatiesGallery }}%</div>
            <div v-if='token.royalties.roylatiesCollector0' class="uppercase tracking-widest text-xs">First Collector: {{ token.royalties.roylatiesCollector0 }} %</div> 
            <div v-if='token.royalties.roylatiesCollector1' class=" uppercase tracking-widest text-xs">Second Collector: {{ token.royalties.roylatiesCollector1 }} %</div>
            <div v-if='token.royalties.roylatiesCollector2' class="uppercase tracking-widest text-xs">Third Collector: {{ token.royalties.roylatiesCollector2 }} %</div>
            <div v-if='token.royalties.roylatiesCollectorX' class="uppercase tracking-widest text-xs">X Collector: {{ token.royalties.roylatiesCollectorX }} %</div>
        ...
        <div class="text-right col-span-full justify-center flex pt-10">
          <button class="mx-2 my-2 mr-10  bg-gray-100 transition duration-150 ease-in-out hover:bg-gray-200 rounded border border-gray-300 text-gray-600 px-6 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-600 xs:mx-2">
